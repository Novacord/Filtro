...
const postUser = async (data) => {
    ...
    let res = await (await fetch("http://localhost:4005/clientes", config)).json()
}

const deleteUser = async(id)=>{
    config.method = "DELETE"
    let res = await ( await fetch(`http://localhost:4005/clientes/${id}`,config)).json()
    console.log(res)
}
...
const putUser = async(data)=>{
    config.method = "PUT";
    config.body = JSON.stringify(data);
    let res = await ( await fetch(`ttp://localhost:4005/clientes/${id}`,config)).json();
}
